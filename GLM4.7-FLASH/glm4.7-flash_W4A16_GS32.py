"""
GLM-4.7-Flash W4A16 Quantization Script - vLLM Compatible Version 2

FIXES the AWQ smoothing error by NOT ignoring shared_experts.
The previous error occurred because ignored layers weren't in AWQ's 
orig_layer_weights dict, but smoothing still tried to process them.

Key changes:
1. Uses glm4_moe_lite_v2.py for vLLM-compatible expert naming
2. Does NOT ignore shared_experts (so AWQ can track and smooth them)
3. Only ignores: lm_head, layer 0, and router gates
"""
...
# Use V2 calibration module for vLLM-compatible weight names
...
# =========================
# Quantization recipe - W4A16
# =========================
# SIMPLIFIED ignore list - DO NOT ignore shared_experts!
# The AWQ smoothing error occurred because ignored layers weren't tracked
# but smoothing regex patterns still matched them.
#
# We only ignore:
# - lm_head (output projection)
# - layer 0 (dense layer - keep high precision for stability)
...
# NOTE: We do NOT ignore shared_experts anymore!
# This ensures AWQ can track and smooth them properly.
...
# Remove references to avoid shared tensor issues
...
print(f"\nW4A16: 4-bit integer weights, 16-bit float activations")
print(f"Expected size: ~15-17 GB")
print(f"\nvLLM command:")
