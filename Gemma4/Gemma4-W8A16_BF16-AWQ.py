"""
Gemma 4: BF16 attention + W4A16 AWQ (MLP only)

  Per-head q_norm/k_norm/v_norm make attention AWQ mappings harmful (#2522). This script
  leaves self_attn q/k/v/o_proj in BF16 (no weight quantization on those Linears).

  AWQModifier targets only mlp gate/up/down_proj with the usual Gemma 4 mappings:
  pre_feedforward_layernorm -> gate/up, up -> down. duo_scaling=\"both\", n_grid=40.

  W4 weights: symmetric int4, group size (default 32), Marlin-friendly.
  ...
  Example:
    python Gemma4-W4A16_BF16-AWQ.py /path/to/gemma-4-31B/ /path/to/out/ \\
...
parser = argparse.ArgumentParser(
    description="Gemma 4: BF16 attention Linears + W4A16 AWQ on MLP only."
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="AWQ loss on assistant tokens only; enables pipeline=sequential.",
)
...
print(
    "Quantization targets: MLP gate/up/down_proj W4 only; "
    "self_attn q/k/v/o_proj stay BF16 (not in AWQ targets)."
)
...
# =========================
# AWQ on MLP only; attention Linears excluded (remain BF16)
# =========================
_RECIPE_IGNORE = [
    "lm_head",
    "re:.*vision_tower.*",
...
print(
    f"\n=== Running W4A16 AWQ (MLP only; attn BF16) "
    f"(group_size={args.group_size}, symmetric=True, "
    f"duo_scaling=both, n_grid=40, use_loss_mask={use_loss_mask}) ==="
)
oneshot_kwargs = dict(
    model=model,
    dataset=ds,
