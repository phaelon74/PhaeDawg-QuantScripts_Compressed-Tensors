"""
Gemma 4 W4A16: GPTQ (attention) + AWQ (MLP)

  Per-head q_norm/k_norm/v_norm after every attention projection erase AWQ smoothing on
  those layers (llm-compressor #2522). AWQ-only leaves ~56% of Linear layers as plain RTN.
  This script splits the recipe (see llm-compressor examples/quantization_non_uniform):

  - GPTQModifier on self_attn q/k/v/o_proj — Hessian-based W4 where AWQ cannot help.
  - AWQModifier on mlp gate/up/down_proj — custom Gemma 4 MLP mappings only
    (pre_feedforward_layernorm -> gate/up, up -> down).

  - config_groups for group size (default 32), symmetric int4 (Marlin-friendly)
  - duo_scaling=\"both\", n_grid=40 on AWQ for smoother grid search
  - pipeline=sequential always (required for multi-modifier + loss mask)
  ...
  Example:
    python Gemma4-W4A16_AWQ_GPTQ.py /path/to/gemma-4-31B/ /path/to/out/ \\
      ../Recipes/Datasets/General_reasoning.yaml --group-size 32

  Optional --use-loss-mask: calibration loss on assistant tokens only (sequential pipeline).
...
parser = argparse.ArgumentParser(
    description="Gemma 4 W4A16: GPTQ (attention) + AWQ (MLP), recipe YAML + group size."
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Calibration loss on assistant tokens only (sequential pipeline; required for multi-modifier).",
)
...
# =========================
# GPTQ (attention) + AWQ (MLP): sequential pipeline, shared W4 config
# =========================
_RECIPE_IGNORE = [
    "lm_head",
    "re:.*vision_tower.*",
...
recipe = [
    GPTQModifier(
        ...
        config_groups=_w4_config_groups,
    ),
    AWQModifier(
...
print(
    f"\n=== Running W4A16 GPTQ(attn) + AWQ(mlp) "
    f"(group_size={args.group_size}, symmetric=True, "
    f"duo_scaling=both, n_grid=40, pipeline=sequential, use_loss_mask={use_loss_mask}) ==="
)
oneshot_kwargs = dict(
    model=model,
    dataset=ds,
