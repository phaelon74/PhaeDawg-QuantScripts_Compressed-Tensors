...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="GPTQ loss on assistant tokens only; enables pipeline=sequential.",
)
...
def messages_to_calibration_text(tokenizer, messages) -> str:
    """Use chat template when present; otherwise plain multi-turn text."""
    if not messages:
        return ""
...
def _align_loss_mask_to_seq(mask, seq_len, pad=0):
    """Must equal len(input_ids) or sequential pipeline hooks get mismatched [B,T] vs activations."""
    m = list(mask)[:seq_len]
    if len(m) < seq_len:
