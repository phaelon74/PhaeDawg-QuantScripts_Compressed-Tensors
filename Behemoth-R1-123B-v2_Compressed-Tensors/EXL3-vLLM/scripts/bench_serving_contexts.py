...
def make_prompt_tokens(
    ...
    # A random prefix changes the first KV-cache block on every request.
    nonce = " ".join(uuid.uuid4().hex for _ in range(8))
    prefix = client.tokenize(model, nonce)
    filler = client.tokenize(
        model,
        " Neutral benchmark context used to measure language model serving.",
    )
    if not filler:
        raise RuntimeError("Tokenizer returned no filler tokens")
    if len(prefix) >= target_tokens:
        return prefix[:target_tokens]
    repeats = math.ceil((target_tokens - len(prefix)) / len(filler))
    return (prefix + filler * repeats)[:target_tokens]
...
def main() -> int:
    ...
    metadata = {
        ...
        "prompt_format": "exact token IDs generated by server /tokenize",
